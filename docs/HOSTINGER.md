...
Un VPS permet d’exécuter le web Next.js, le worker BullMQ, PostgreSQL, Redis, MariaDB et le WordPress local de validation. C’est la topologie la plus proche de l’environnement `docker-compose.yml` et la seule qui conserve toutes les fonctions locales du MVP sur une même machine.

Le déploiement devra exposer uniquement l’application web derrière HTTPS. PostgreSQL, Redis, MariaDB, Docker et le port du worker restent privés. Le répertoire défini par `ARTIFACTS_DIR` doit être placé sur un volume persistant et sauvegardé.

## Hébergement Web ou Cloud géré

Hostinger prend en charge Next.js, pnpm et Node.js 24 depuis un dépôt GitHub. En revanche, Redis n’est pas disponible sur les offres Web/Cloud gérées. Cette topologie exige donc :

- un PostgreSQL externe compatible Prisma, par exemple Supabase ;
- un Redis externe accessible en TLS avec une URL `rediss://` ;
- un worker BullMQ exécuté sur un service séparé ou un VPS ;
...
Ne déployez pas uniquement le web en pensant que les workflows avanceront : sans worker connecté à la même base, au même Redis et au même stockage d’artefacts, les travaux resteront dans la file.
...
Le contrôle vérifie les URLs PostgreSQL/Redis, HTTPS pour `APP_URL`, les secrets de session et de chiffrement, le stockage et les paramètres des fournisseurs réels. Il n’affiche jamais la valeur des secrets.
...
- [Disponibilité de Redis chez Hostinger](https://www.hostinger.com/support/9581774-is-redis-supported-at-hostinger/)

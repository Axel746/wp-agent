...
export async function validateRemoteUrl(raw: string, options: { allowPrivate?: boolean; production?: boolean } = {}): Promise<URL> {
  let url: URL;
...
export async function safeFetch(raw: string, init: RequestInit = {}, options: { allowPrivate?: boolean; production?: boolean; maxRedirects?: number; maxBytes?: number; timeoutMs?: number } = {}): Promise<Response> {
  ...
  let url = await validateRemoteUrl(raw, options);
  for (let redirect = 0; redirect <= maxRedirects; redirect++) {
    const response = await fetch(url, { ...init, redirect: "manual", signal: AbortSignal.timeout(options.timeoutMs ?? 10_000) });
    if ([301, 302, 303, 307, 308].includes(response.status)) {
      const location = response.headers.get("location");
      if (!location || redirect === maxRedirects) throw new AppError("REDIRECT_BLOCKED", "Trop de redirections WordPress");
      url = await validateRemoteUrl(new URL(location, url).toString(), options);
      continue;
    }
